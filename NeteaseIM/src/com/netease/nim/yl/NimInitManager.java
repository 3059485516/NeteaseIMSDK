...
import com.netease.nim.yl.event.OnlineStateEventManager;
...
public class NimInitManager {
    ...
    private void registerRTSIncomingObserver(boolean register) {
        RTSManager.getInstance().observeIncomingSession(new Observer<RTSData>() {
            ...
            public void onEvent(RTSData rtsData) {
                RTSActivity.incomingSession(NetIMCache.getContext(), rtsData, RTSActivity.FROM_BROADCAST_RECEIVER);
            }
        }, register);
    }
    ...
    private void registerLocaleReceiver(boolean register) {
        if (register) {
            ...
            NetIMCache.getContext().registerReceiver(localeReceiver, filter);
        } else {
            NetIMCache.getContext().unregisterReceiver(localeReceiver);
        }
    }
    ...
    private void updateLocale() {
        Context context = NetIMCache.getContext();
    ...
    private void registerBroadcastMessages(boolean register) {
        NIMClient.getService(MsgServiceObserve.class).observeBroadcastMessage(new Observer<BroadcastMessage>() {
            ...
            public void onEvent(BroadcastMessage broadcastMessage) {
                Toast.makeText(NetIMCache.getContext(), "收到全员广播 ：" +  broadcastMessage.getContent(), Toast.LENGTH_SHORT).show();
            }
        }, register);
    }
