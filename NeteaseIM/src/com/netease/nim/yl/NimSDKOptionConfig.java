...
public class NimSDKOptionConfig {
    ...
    public static SDKOptions getSDKOptions(Context context,Class cls) {
        ...
        initStatusBarNotificationConfig(options, cls);
        // 配置保存图片，文件，log等数据的目录
        ...
        // 用户信息提供者
        options.userInfoProvider = new NimUserInfoProvider(NetIMCache.getContext());
    ...
    public static String getAppCacheDir(Context context) {
        ...
        if (TextUtils.isEmpty(storageRootPath)) {
            ...
            storageRootPath = Environment.getExternalStorageDirectory() + "/" + NetIMCache.getContext().getPackageName();
        }
    ...
    private static void initStatusBarNotificationConfig(SDKOptions options, Class cls) {
        // load 应用的状态栏配置
        StatusBarNotificationConfig config = loadStatusBarNotificationConfig(cls);
    ...
    private static StatusBarNotificationConfig loadStatusBarNotificationConfig(Class cls) {
        ...
        // 点击通知需要跳转到的界面
        config.notificationEntrance = cls;
        config.notificationSmallIconId = R.drawable.ic_stat_notify_msg;
        config.notificationColor = NetIMCache.getContext().getResources().getColor(R.color.color_blue_3a9efb);
        // 通知铃声的uri字符串
        config.notificationSound = "android.resource://com.yl.lovestudy/raw/avchat_ring";
        ...
        // save cache，留做切换账号备用
        NetIMCache.setNotificationConfig(config);
        return config;
    }
