package com.netease.nim.yl.location.activity;
...
import com.netease.nim.yl.location.helper.NimGeocoder;
import com.netease.nim.yl.location.helper.NimLocationManager;
import com.netease.nim.yl.location.helper.NimLocationManager.NimLocationListener;
import com.netease.nim.yl.location.model.NimLocation;
...
//	private static final String TAG = "LocationAmapActivity";
    ...
    private void updateSendStatus() {
    	...
    	if(btnMyLocation.getVisibility() == View.VISIBLE || Math.abs(-1 - cacheLatitude) < 0.1f) {
			...
			mTvTitle.setText(R.string.my_location);
    	}
    }
    ...
    public void onClick(View v) {
        int viewId = v.getId();
        if (viewId == R.id.action_bar_right_clickable_textview) {
			sendLocation();
			finish();
		}else if (viewId == R.id.location_pin) {
			setPinInfoPanel(!isPinInfoPanelShow());
		}else if (viewId == R.id.location_info) {
			pinInfoPanel.setVisibility(View.GONE);
		}else if (viewId == R.id.my_location) {
