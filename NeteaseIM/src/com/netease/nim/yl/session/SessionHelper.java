package com.netease.nim.yl.session;
...
import com.netease.nim.yl.contact.activity.RobotProfileActivity;
import com.netease.nim.yl.contact.activity.UserProfileActivity;
import com.netease.nim.yl.session.action.AVChatAction;
import com.netease.nim.yl.session.action.FileAction;
import com.netease.nim.yl.session.action.GuessAction;
import com.netease.nim.yl.session.action.TeamAVChatAction;
import com.netease.nim.yl.session.activity.MessageHistoryActivity;
import com.netease.nim.yl.session.activity.MessageInfoActivity;
import com.netease.nim.yl.session.extension.CustomAttachParser;
import com.netease.nim.yl.session.extension.CustomAttachment;
import com.netease.nim.yl.session.extension.GuessAttachment;
import com.netease.nim.yl.session.extension.RTSAttachment;
import com.netease.nim.yl.session.extension.RedPacketAttachment;
import com.netease.nim.yl.session.extension.SnapChatAttachment;
import com.netease.nim.yl.session.extension.StickerAttachment;
import com.netease.nim.yl.session.search.SearchMessageActivity;
import com.netease.nim.yl.session.viewholder.MsgViewHolderAVChat;
import com.netease.nim.yl.session.viewholder.MsgViewHolderDefCustom;
import com.netease.nim.yl.session.viewholder.MsgViewHolderFile;
import com.netease.nim.yl.session.viewholder.MsgViewHolderGuess;
import com.netease.nim.yl.session.viewholder.MsgViewHolderRTS;
import com.netease.nim.yl.session.viewholder.MsgViewHolderSnapChat;
import com.netease.nim.yl.session.viewholder.MsgViewHolderSticker;
import com.netease.nim.yl.session.viewholder.MsgViewHolderTip;
...
public class SessionHelper {
     ...
     * 新增
     */
    public static void startP2PSession2(Context context, String account, Uri uri) {
        startP2PSession2(context, account, null, uri);
    }

    /**
     * 新增
     */
    public static void startP2PSession2(Context context, String account, IMMessage anchor, Uri data) {
        if (!NetIMCache.getAccount().equals(account)) {
            if (NimUIKit.getRobotInfoProvider().getRobotByAccount(account) != null) {
                NimUIKit.startChatting(context, account, SessionTypeEnum.P2P, getRobotCustomization(), anchor, data);
            } else {
                NimUIKit.startP2PSession(context, account, anchor,data);
            }
        } else {
            NimUIKit.startChatting(context, account, SessionTypeEnum.P2P, getMyP2pCustomization(), anchor, data);
        }
    }
     ...
     * 新增
     */
    public static void startTeamSession(Context context, String tid, IMMessage anchor, Uri data) {
        NimUIKit.startTeamSession(context, tid, anchor, data);
    }
...
//            p2pCustomization.backgroundUri = "android.resource://com.netease.nim.demo/drawable/bk"
            ...
            cloudMsgButton.iconId = R.mipmap.his;
            ...
            infoButton.iconId = R.mipmap.add_c;
...
//            p2pCustomization.backgroundUri = "android.resource://com.netease.nim.demo/drawable/bk"
    ...
    private static SessionCustomization getTeamCustomization() {
        if (teamCustomization == null) {
            ...
            infoButton.iconId = R.mipmap.team;
            buttons.add(cloudMsgButton);
            buttons.add(infoButton);
        ...
        NimUIKit.setMsgRevokeFilter(new MsgRevokeFilter() {
            ...
            public boolean shouldIgnore(IMMessage message) {
                if (message.getAttachment() != null
                    ...
                    return true;
                } else if (NetIMCache.getAccount().equals(message.getSessionId())) {
                    // 发给我的电脑 不允许撤回
                    return true;
                }
                return false;
    ...
    private static List<PopupMenuItem> getMoreMenuItems(Context context, String sessionId, SessionTypeEnum sessionTypeEnum) {
        ...
        moreMenuItems.add(new PopupMenuItem(context, ACTION_HISTORY_QUERY, sessionId,
                sessionTypeEnum, NetIMCache.getContext().getString(R.string.message_history_query)));
        moreMenuItems.add(new PopupMenuItem(context, ACTION_SEARCH_MESSAGE, sessionId,
                sessionTypeEnum, NetIMCache.getContext().getString(R.string.message_search_title)));
        moreMenuItems.add(new PopupMenuItem(context, ACTION_CLEAR_MESSAGE, sessionId,
                sessionTypeEnum, NetIMCache.getContext().getString(R.string.message_clear)));
