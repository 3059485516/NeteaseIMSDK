...
public class AdvancedTeamMemberInfoActivity extends UI implements View.OnClickListener {
    ...
    //好友关系
    private Button teamMemberRelation;
    ...
    private void findViews() {
        ...
        teamMemberRelation = (Button) findViewById(R.id.team_member_relation);
    ...
    private void showConfirmButton() {
        ...
        final EasyAlertDialog dialog = EasyAlertDialogHelper.createOkCancelDiolag(this, null, getString(R.string.team_member_remove_confirm), getString(R.string.remove), getString(R.string.cancel), true, listener);
        dialog.show();
    }
     ...
     * 判断成员是否是自己的好友
     */
    private void teamMemberRelationStatus() {
        boolean isSelf = isSelf(account);
        if (isSelf) {
            teamMemberRelation.setVisibility(View.GONE);
        } else {
            teamMemberRelation.setVisibility(View.VISIBLE);
        }
        isChargeMyFriend();
        teamMemberRelation.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                ...
                if (isMyFriend){
                    ...
                    addNeteaseIMFriend(account);
                }
            }
        });
    }

    private void isChargeMyFriend(){
        boolean isMyFriend = NIMClient.getService(FriendService.class).isMyFriend(account);
        if (isMyFriend) {
            teamMemberRelation.setText(getText(R.string.team_start_chat));
        }else {
            teamMemberRelation.setText(getText(R.string.team_add_friend));
        }
    }


    private void addNeteaseIMFriend(String imAccount) {
        final VerifyType verifyType = VerifyType.DIRECT_ADD; // 发起好友验证请求
        String msg = "群内成员发出邀请";
        DialogMaker.showProgressDialog(this, msg, true);
        NIMClient.getService(FriendService.class).addFriend(new AddFriendData(imAccount, verifyType, msg)).setCallback(new RequestCallback<Void>() {
            @Override
            ...
            public void onException(Throwable throwable) {
                DialogMaker.dismissProgressDialog();
                toast("添加好友失败：" + throwable.getMessage());
            }
        });
    }
    ...
    private void toast(String toastStr) {
        Toast.makeText(AdvancedTeamMemberInfoActivity.this, toastStr, Toast.LENGTH_LONG).show();
    }
}
