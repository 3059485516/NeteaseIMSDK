...
public class MsgViewHolderText extends MsgViewHolderBase {
    ...
    protected void bindContentView() {
        ...
        bodyTextView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                Map map = message.getRemoteExtension();
                  if (map!=null){
                    String type =  (String) map.get("type");
                    if ("pushHealth".equals(type)){
                        String studId = (String) map.get("stud_uuid");
                        String role = "student";
                        if (TextUtils.isEmpty(studId)) {
                            studId = NimUIKit.getAccount();
                            role = "teacher";
                        }
                         onNewItemClick(role,studId);
                    }else {
                        onItemClick();
                    }
                }else {
    ...
    private void onNewItemClick(String role,String uuid){
        Intent intent= new Intent();
        intent.setAction("love.action.health");
        intent.putExtra("type",role);//student,teacher
